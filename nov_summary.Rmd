...
for (row in 1:dim(model_sum)[1]){
  ...
  for (i in 1:100){
      ...
      fit <- glm(outcome_int ~ risk_t1, data = c, family = binomial)
      y.pred <- predict(fit, as.data.frame(c), type = "response")
      # mse[i] <- mean((c$outcome_int - y.pred)^2)
      mse = mse + mean((c$outcome_int - y.pred)^2)
      mse_true = mse_true + mean((c$outcome_noint - y.pred)^2)
      auc = auc + auc(c$outcome_int, y.pred)
      auc_true = auc_true + auc(c$outcome_noint, y.pred)
  }
  model_sum$mean_squared_error[row] <- mse/100
  model_sum$mean_squared_error_true[row] <- mse_true/100
  model_sum$auc[row] <- auc/100
  model_sum$auc_true[row] <- auc_true/100
...
for (p in 1:3) {
  ...
  for (i in 1:10){
    for (j in 1:10){
      ...
      mse_table[i,j] <- cur_mse/new_repeats
      brier_table[i,j] <- cur_brier/new_repeats
