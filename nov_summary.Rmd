...
axis(side=1, at=seq(-1,3,0.2), labels=seq(-1,3,0.2))
hist(calibrationgrid_list[[3]], xlim = c(-1,4), breaks = seq(-1,4,0.2))
axis(side=1, at=seq(-1,3,0.2), labels=seq(-1,3,0.2))
```
...
For low prevalence disease, even when treatment is generally effective, few outcomes are changed with intervention. Therefore, as prevalence increases, the prediction power depends more on the internvention effectiveness. 
...
For low to mid prevalent diseases, there are more underestimation. For high prevalent disease, there are signs of overestimating the probability of outcome when a treatment is more effective; however, when treatment is less effective, the model becomes better calibrated.
