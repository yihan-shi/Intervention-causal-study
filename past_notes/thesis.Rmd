...
```{r}
gen_cmi_2 <- function(risk_t1, n = n, prevalence, cmi_fraction){
  df <- gen_outcome1(risk_t1, n, prevalence)

  # xb <- prevalence + 5 * risk_t1 # 5 is a tuning parameter: degree of connection between risk and cmi; 0: not dependent on risk_t1
  xb <- prevalence + cmi_fraction * risk_t1 
  p <- exp(xb)/(1 + exp(xb))
...
# df <- gen_cmi_2(risk_t1, n, prevalence[1], 100)
# mean(df$cmi)

gen_risk_t2_2 <- function(risk_t1, n = n, b_intervention, prevalence, cmi_fraction){
  # if cmi == 1 & outcome_noint == 1, then risk_t2 = risk_t1 * b_intervention
  df <- gen_cmi_2(risk_t1, n, prevalence, cmi_fraction)
  df <- df %>%
    mutate(risk_t2 = ifelse((outcome_noint == 1 & cmi == 1),
                            risk_t1 * b_intervention,
                            risk_t1))
...
gen_outcome_f2 <- function(risk_t1, n = n, b_intervention,prevalence, cmi_fraction){
  df <- gen_risk_t2_2(risk_t1, n, b_intervention, prevalence, cmi_fraction)
  condition <- df %>%
    filter(outcome_noint == 1 & cmi == 1)
...
# df <- gen_outcome_f2(risk_t1, n, 0.1, prevalence[1], 100)
```
...
cmi_fractionpop <- matrix(NA, 3, 9) # calculate real cmi fraction in population
...
for (p in 1:3){
  for (j in 1:9){
      c <- as.data.frame(gen_outcome_f2(risk_t1,
                         ...
                         b_intervention = i/10,
                         prevalence = prevalence[p],
                         cmi_fraction = cmi_fraction[j]))
      cmi_fractionpop[p,j] <- mean(c$cmi)
  }
}
saveRDS(cmi_fractionpop, "../data/causal2_cmi_fractionpop", compress = "gzip")
...
for (p in 1:3) {
  ...
  auc_table <- matrix(NA, nrow = length(b_int), ncol = length(cmi_fraction))
  calibration_table <- matrix(NA, nrow = length(b_int), ncol = length(cmi_fraction))
  ...
  for (i in 1:9){
    ...
    auc_list[[p]] <- auc_table
    calibration_list[[p]] <- calibration_table
}

# save objects
# saveRDS(ideal_auc, "../data/causal2_ideal_auc", compress = "gzip")
# saveRDS(cmi_perclist, "causal2_cmi_perc_eval", compress = "gzip")
saveRDS(auc_list, "../data/causal2_auc_eval", compress = "gzip")
saveRDS(calibration_list, "../data/causal2_calibration_eval", compress = "gzip")
...
# for (row in 111:111) {
  ...
  # for (i in 1:6){
    ...
    auc_uncorrected_bias = auc_uncorrected_bias + (ideal_auc - auc(y.test, x.test$risk_t1)) # old bias
    ...
    auc_corrected_bias =  auc_corrected_bias + (ideal_auc - auc(y.test, x.test_2$new_risk)) # new bias
...
# for (row in 1:1){
  ...
  # for (i in 20:20) {
    ...
    auc_corrected_bias = auc_corrected_bias + 
      dim(have_cmi)[1]/dim(df)[1] * (ideal_auc - auc(have_cmi$outcome_int, have_cmi$risk_t1)) + 
      dim(no_cmi)[1]/dim(df)[1] * (ideal_auc - auc(no_cmi$outcome_int, no_cmi$risk_t1))
    
    auc_uncorrected_bias = auc_uncorrected_bias + (ideal_auc - auc(df$outcome_int, df$risk_t1))
  }
