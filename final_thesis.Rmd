...
#### 2.1.1 Model evalucation under varying intervention effectiveness $\beta_{intervention}$ and `cmi_cutoff`
...
$$
\text{AUC} = \beta_0 + \beta_1 \times \text{b$_{intervention}$} + \beta_2 \times \text{prevalence}
$$
...
Simulated observations are divided into two groups: intervened (`cmi` = 1) and non-intervened (`cmi` = 0). In each group, a binary logistic regression model is fit on the final outcome using the knowledge of initial risk `risk_t1`.
