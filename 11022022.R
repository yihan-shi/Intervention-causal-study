...
gen_cmi <- function(risk_t1, n = n, cmi_cutoff){
  ...
  df <- df %>%
    mutate(cmi = case_when(risk_t1 > cmi_cutoff ~ 1,
                           TRUE ~ 0))
...
a <- gen_cmi(risk_t1, n, cmi_cutoff)
...
gen_risk_t2 <- function(risk_t1, n = n, b_intervention, cmi_cutoff){
  ...
  df <- df %>%
    mutate(risk_t2 = case_when((outcome_noint == 1 & cmi == 1) ~ risk_t1 * b_intervention,
                               TRUE ~ risk_t1))
  return (df)
}
b <- gen_risk_t2(risk_t1, n, b_intervention, cmi_cutoff)
